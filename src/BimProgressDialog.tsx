...
import { Boxes, CheckCircle2, FileCheck2, Filter, Layers3, ScanLine, Upload, X } from 'lucide-react'
...
export default function BimProgressDialog({workPackages,initialWorkPackageId,previousPct,preparedBy,initialEvidence,onApply,onClose}:Props) {
  ...
  const loadExample=(id:string)=>{
    ...
    setModelVersion(next.modelVersion); setIfcFile(undefined); setIfcElements([]); setCategory('Alle categorieën'); setStorey('Alle zones')
  }
  ...
  return <div className="modal-backdrop bim-progress-backdrop">
    <section className="modal bim-progress-dialog" aria-label="BIM-vordering opmaken">
      <header className="modal-head"><div><p className="eyebrow">4D/5D voortgangsmeting</p><h2>BIM-vordering samenstellen</h2><span>Selecteer uitgevoerde modelelementen, verifieer de hoeveelheid en boek ze op een werkpakket.</span></div><button className="icon-button" aria-label="Sluiten" onClick={onClose}><X size={20}/></button></header>
      <div className="bim-progress-layout">
        ...
        <main className="bim-progress-viewer">
          <div className="bim-progress-toolbar"><label><Filter size={14}/><select value={category} onChange={event=>setCategory(event.target.value)}><option>Alle categorieën</option>{categories.map(item=><option key={item}>{item}</option>)}</select></label><label><select value={storey} onChange={event=>setStorey(event.target.value)}><option>Alle zones</option>{storeys.map(item=><option key={item}>{item}</option>)}</select></label><button className="secondary" onClick={()=>setSelected(new Set(visible.map(item=>item.id)))}>Selecteer zichtbaar</button><button className="secondary" onClick={()=>setSelected(new Set())}>Wis selectie</button></div>
          <div className="bim-progress-canvas">
            {ifcFile?<Suspense fallback={<div className="bim-progress-loading">IFC-engine laden…</div>}><BimIfcViewer file={ifcFile} selectedExpressIds={new Set([...selected].map(Number))} onSelectionChange={ids=>setSelected(new Set([...ids].map(String)))} onProgress={setIfcProgress} onError={()=>setIfcProgress(0)} onModelLoaded={report=>{setIfcElements(report.elements);const imported=report.elements.map((item):BimProgressElement=>({id:String(item.expressId),code:`IFC-${item.expressId}`,label:item.name,category:item.category,storey:item.storey,quantity:item.quantity,unit:item.unit,plannedProgressPct:100,completedProgressPct:0,verified:!item.warning}));setElements(imported);setSelected(new Set(imported.slice(0,Math.min(24,imported.length)).map(item=>item.id)));setIfcProgress(100)}} /></Suspense>:<div className="bim-progress-grid">{visible.map((item,index)=><button key={item.id} className={`${selected.has(item.id)?'selected':''} ${item.verified?'verified':'review'}`} style={{'--bim-index':index} as React.CSSProperties} onClick={()=>setSelected(current=>{const next=new Set(current);if(next.has(item.id))next.delete(item.id);else next.add(item.id);return next})}><span>{item.storey}</span><Boxes size={18}/><strong>{item.code}</strong><small>{item.category}<br/>{number(item.quantity)} {item.unit}</small>{item.verified&&<CheckCircle2 size={13}/>}</button>)}</div>}
