...
export default function BimProgressDialog({projectId,lidarPersistence,workPackages,initialWorkPackageId,previousPct,preparedBy,initialEvidence,onApply,onClose}:Props) {
  ...
  const loadExample=(id:string)=>{
    ...
    setModelVersion(next.modelVersion); setIfcFile(undefined); setIfcElements([]); setCategory('Alle categorieën'); setStorey('Alle zones'); setTimelineIndex(0); setDimension(next.elements.some(item=>item.geometry)?'3D':'5D')
  }
