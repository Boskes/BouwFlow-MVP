...
import LidarArtifactGallery from './LidarArtifactGallery'
...
export type LidarPersistence={
  ...
  upload:(scanId:string,file:File,input:{kind:LidarArtifact['kind'];capturedAt:string})=>Promise<LidarScanSession|undefined>;downloadArtifact:(scanId:string,artifactId:string)=>Promise<Blob|undefined>;register:(scanId:string,points:LidarControlPoint[],by:string)=>Promise<LidarScanSession|undefined>;
  ...
  return <div className="modal-backdrop lidar-workspace-backdrop">
    <section className="modal lidar-workspace" aria-label="LiDAR BIM-voortgang">
      ...
      <div className="lidar-body">
        <aside className="lidar-side"><div className="lidar-device-card"><Smartphone size={22}/><span><strong>{session.deviceName}</strong><small>{session.captureMode} · LiDAR {session.deviceSupportsLidar?'actief':'niet beschikbaar'}</small></span><CheckCircle2 size={16}/></div><LidarArtifactGallery key={session.id} compact scanId={session.id} artifacts={session.artifacts} downloadArtifact={props.persistence?.downloadArtifact}/><input hidden ref={uploadRef} type="file" accept=".usdz,.json,.ply,.obj,.zip,image/*" onChange={event=>importArtifact(event.target.files?.[0])}/><button className="secondary" onClick={()=>uploadRef.current?.click()}><CloudUpload size={14}/>Scanbestand importeren</button><div className="lidar-native"><Smartphone size={17}/><span><strong>BouwFlow Scan voor iPhone</strong><small>RoomPlan/ARKit-opnames synchroniseren met dezelfde Entra-aanmelding.</small></span></div></aside>
