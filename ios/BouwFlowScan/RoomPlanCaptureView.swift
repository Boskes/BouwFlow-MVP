...
struct RoomPlanCaptureView: UIViewRepresentable {
    ...
    let onElements: ([SurveyElementPayload]) -> Void
    let onError: (Error) -> Void

    func makeCoordinator() -> Coordinator { Coordinator(onArtifacts: onArtifacts, onElements: onElements, onError: onError) }
    ...
    final class Coordinator: NSObject, RoomCaptureViewDelegate {
        ...
        let onElements: ([SurveyElementPayload]) -> Void
        let onError: (Error) -> Void
        init(onArtifacts: @escaping ([LocalScanArtifact]) -> Void, onElements: @escaping ([SurveyElementPayload]) -> Void, onError: @escaping (Error) -> Void) { self.onArtifacts = onArtifacts; self.onElements = onElements; self.onError = onError }
        ...
        func captureView(didPresent processedResult: CapturedRoom, error: Error?) {
            ...
            do {
                ...
                onElements(RoomPlanSurveyMapper.elements(from: processedResult))
