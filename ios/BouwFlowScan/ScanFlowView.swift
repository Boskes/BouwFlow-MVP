        ...
        NavigationStack {
            Form {
                ...
                Section("Fase 1 · LiDAR-opname") {
                    if RoomCaptureSession.isSupported { RoomPlanCaptureView(running: $running, onArtifacts: { artifacts.append(contentsOf: $0); status = "Opname klaar: \(artifacts.count) bewijsbestanden" }, onError: { status = $0.localizedDescription }).frame(minHeight: 330).clipShape(RoundedRectangle(cornerRadius: 12)) } else { ContentUnavailableView("LiDAR niet beschikbaar", systemImage: "iphone.slash", description: Text("Gebruik een ondersteunde iPhone Pro of iPad Pro.")) }
