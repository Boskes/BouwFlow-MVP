...
export interface BuildAppOptions {
  ...
  checkinatworkGateway?: CheckinatworkGateway
}
...
export async function buildApp({ pool, authMode = 'development', logger = false, frontendOrigin = 'http://localhost:5173', objectStorage = new LocalObjectStorage(), peppolValidator = createPeppolValidator(), peppolAccessPoint = createPeppolAccessPoint(), peppolWebhookSecret = process.env.PEPPOL_WEBHOOK_SECRET ?? '', peppolWebhookPublicUrl = process.env.PEPPOL_WEBHOOK_PUBLIC_URL ?? '', peppolStatusPollIntervalMs, peppolNotificationSender, peppolNotificationTargets: configuredNotificationTargets, peppolNotificationDispatchIntervalMs, peppolCriticalSlaMinutes = Number(process.env.PEPPOL_CRITICAL_SLA_MINUTES ?? 15), trustProxy = false, rateLimitMax = 5_000, rateLimitWindowMs = 60_000, release = 'development', requireIdempotencyKey = false, integrationGateway = createIntegrationGateway(requireIdempotencyKey), aiGateway = createAiGateway(requireIdempotencyKey), quoteMailGateway: configuredQuoteMailGateway, documentMailGateway: configuredDocumentMailGateway, centralMailService: configuredCentralMailService, belgianAddressSearch = new HttpBelgianAddressSearch(), bimTestModelFetch = fetch, priceIndexProvider:configuredPriceIndexProvider, checkinatworkGateway = createCheckinatworkGateway() }: BuildAppOptions) {
  ...
  const repository = new BouwFlowRepository(pool, objectStorage, notificationTargets, criticalSlaMinutes, Boolean(notificationSender), notificationProvider, notificationChannels, integrationChecks, integrationGateway, aiGateway, priceIndexProvider, checkinatworkGateway)
  ...
  app.put('/api/checkinatwork/sites', { preHandler: requireRoles('Administrator','Projectdirecteur','Projectmanager','Werfleider','Preventieadviseur') }, async request => repository.configureCheckinatworkSite(request.context, checkinatworkSiteSchema.parse(request.body)))
  app.post('/api/checkinatwork/participants', { preHandler: requireRoles('Administrator','HR','Projectmanager','Werfleider','Preventieadviseur','Onderaannemer') }, async (request, reply) => reply.code(201).send(await repository.createCheckinatworkParticipant(request.context, checkinatworkParticipantSchema.parse(request.body))))
  app.post('/api/checkinatwork/registrations', { preHandler: requireRoles('Administrator','Projectmanager','Werfleider','Ploegbaas','Arbeider','Onderaannemer','Preventieadviseur') }, async (request, reply) => reply.code(201).send(await repository.registerCheckinatworkPresence(request.context, checkinatworkRegistrationSchema.parse(request.body))))
  app.post('/api/checkinatwork/registrations/:id/cancel', { preHandler: requireRoles('Administrator','Projectmanager','Werfleider','Preventieadviseur','Onderaannemer') }, async request => repository.cancelCheckinatworkPresence(request.context, uuidParams.parse(request.params).id, checkinatworkCancellationSchema.parse(request.body).reason))
