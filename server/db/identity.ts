...
interface IdentityRow { id:string; role:string; status:string; entra_object_id:string|null }

async function configuredIdentity(pool:Pick<Pool,'query'>,context:RequestContext){
  const result=await pool.query<IdentityRow>(`SELECT id,role,status,entra_object_id FROM users WHERE tenant_id=$1 AND (entra_object_id=$2 OR lower(email)=lower($3)) ORDER BY CASE WHEN entra_object_id=$2 THEN 0 ELSE 1 END LIMIT 1`,[context.tenantId,context.userId,context.email])
  const profile=result.rows[0]
  if(!profile)return undefined
  if(profile.status==='Geblokkeerd')throw new AuthenticationError('Deze BouwFlow-account is geblokkeerd')
  const role=profile.entra_object_id===context.userId?(context.roles[0]??profile.role):profile.role
  await pool.query('UPDATE users SET entra_object_id=$3,display_name=$4,email=$5,role=$6,status=CASE WHEN status=\'Uitgenodigd\' THEN \'Actief\' ELSE status END WHERE tenant_id=$1 AND id=$2',[context.tenantId,profile.id,context.userId,context.displayName,context.email,role])
...
export async function ensureAuthenticatedIdentity(pool: Pick<Pool, 'query'>, context: RequestContext, tenantName = 'BouwFlow organisatie') {
  ...
  await pool.query(`INSERT INTO users (tenant_id,id,entra_object_id,display_name,email,role,all_legal_entities,status,all_projects)
    VALUES ($1,$2,$3,$4,$5,$6,$7,'Actief',true)
    ON CONFLICT (tenant_id,id) DO UPDATE SET entra_object_id=EXCLUDED.entra_object_id,display_name=EXCLUDED.display_name,email=EXCLUDED.email,role=EXCLUDED.role`,
  [context.tenantId, context.userId, context.userId, context.displayName, context.email, context.roles[0] ?? 'Gebruiker', initialAllLegalEntities])
}
...
export async function ensureExternalPortalIdentity(pool: Pick<Pool, 'query'>, context: RequestContext, tenantName = 'BouwFlow organisatie') {
  ...
  if(await configuredIdentity(pool,context))return
  await pool.query(`INSERT INTO users (tenant_id,id,entra_object_id,display_name,email,role,all_legal_entities,status,all_projects)
    VALUES ($1,$2,$3,$4,$5,$6,false,'Actief',false)
    ON CONFLICT (tenant_id,id) DO NOTHING`,
  [context.tenantId, context.userId, context.userId, context.displayName, context.email, context.roles[0] ?? 'Gebruiker'])
}

export async function loadCompanyAccessScope(pool: Pick<Pool, 'query'>, context: RequestContext) {
  const user = await pool.query<{ all_legal_entities: boolean; all_projects:boolean; role:string; status:string }>('SELECT all_legal_entities,all_projects,role,status FROM users WHERE tenant_id=$1 AND id=$2', [context.tenantId, context.userId])
  if (!user.rowCount) {
  ...
  if(context.configuredAccess)context.roles=[user.rows[0].role]
